...
if [ "$TF_SHOW_DEBUG_CONSOLE" = "YES" ]; then
...
if [ "$TF_SKIP_RESIGNING_AND_REPROVISIONING" != "YES" ]; then
	echo >> $LOG
...
fi #TF_SKIP_RESIGNING_AND_REPROVISIONING


RELEASE_NOTES_FILE=`mktemp /tmp/Release_Notes.XXXXXXXX`
`cd "${SRCROOT}"; git log -n 1 --pretty=short >"${RELEASE_NOTES_FILE}"`

echo "Added release notes:" >> $LOG
echo "--------------------" >> $LOG
cat $RELEASE_NOTES_FILE >> $LOG
echo "--------------------" >> $LOG

# Now onto selecting the Distribution Lists...
if [ "$TF_SKIP_DISTRIBUTION_LISTS" != "YES" ]; then
	TF_DISTRIBUTION_LISTS_QUOTED=`echo "$TF_DISTRIBUTION_LISTS" | tr "," "\n" | sed 's/$/"/' | sed 's/^/"/' | tr "\n" "," | sed 's/,$//'`	
	TF_DISTRIBUTION_LISTS_DEFAULT_SELECTION_QUOTED=`echo "$TF_DISTRIBUTION_LISTS_DEFAULT_SELECTION" | tr "," "\n" | sed 's/$/"/' | sed 's/^/"/' | tr "\n" "," | sed 's/,$//'`	
	SELECTED_DISTRIBUTION_LISTS_QUOTED=`osascript -e "tell application \"Xcode\"" -e "set selected_profile to {choose from list {$TF_DISTRIBUTION_LISTS_QUOTED} with prompt \"Choose Distribution List(s):\" default items {$TF_DISTRIBUTION_LISTS_DEFAULT_SELECTION_QUOTED} with multiple selections allowed}" -e "end tell" -e "return selected_profile"`
	...
	SELECTED_DISTRIBUTION_LISTS="$TF_DISTRIBUTION_LISTS_DEFAULT_SELECTION"
fi #TF_SKIP_DISTRIBUTION_LISTS

echo >> $LOG
echo "Selected Distribution Lists: '$SELECTED_DISTRIBUTION_LISTS'" >> $LOG

if [ "$TF_SKIP_NOTIFY" != "YES" ]; then
	# Ask if we need to notify the permitted team members of the new build:
	if [ "$TF_DEFAULT_NOTIFY_VALUE" = "True" ]; then
	...
	SHOULD_NOTIFY="$TF_DEFAULT_NOTIFY_VALUE"
fi
...
/usr/bin/curl "http://testflightapp.com/api/builds.json" \
  ...
  -F notes=@"$RELEASE_NOTES_FILE" >> $LOG 2>&1
if [ "$?" -ne 0 ]; then
...
if [ "$TF_DISABLE_OPEN_TESTFLIGHT_DASHBOARD" != "YES" ]; then
	echo >> $LOG
